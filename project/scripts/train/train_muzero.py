"""Train a MuZero-style Splendor agent with self-play + latent MCTS."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train MuZero-style Splendor agent")
    parser.add_argument(
        "--config",
        type=str,
        default="project/configs/muzero/muzero_v1_smoke.yaml",
...
def main() -> None:
    ...
    if args.resume.strip():
        ...
        if stem.startswith("muzero_iter_"):
            maybe_iter = stem.replace("muzero_iter_", "")
            if maybe_iter.isdigit():
                start_iter = int(maybe_iter) + 1

    pbar = tqdm(range(start_iter, iterations + 1), desc="MuZero Training", unit="iter")
    for it in pbar:
        ...
        pbar.set_postfix_str(
            f"games={metrics['games']} buf={metrics['buffer_games']} "
            f"ploss={metrics['policy_loss']:.3f} "
            f"vloss={metrics['value_loss']:.3f} "
            f"rloss={metrics['reward_loss']:.3f} "
            f"{iter_elapsed:.0f}s/iter"
        )

        if it % ckpt_freq == 0:
            checkpoint_path = os.path.join(log_dir, f"muzero_iter_{it:04d}.pt")
